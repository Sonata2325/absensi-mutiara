...
@section('content')
<h1 class="text-2xl font-bold mb-6">Tambah Shift</h1>

<form method="POST" action="{{ route('admin.shifts.store') }}" class="bg-white border rounded-2xl p-6 space-y-4">
    @csrf

    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="md:col-span-2">
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_flexible" id="is_flexible" value="1" class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" {{ old('is_flexible') ? 'checked' : '' }}>
                <div>
        ...
        <div>
            ...
            <input name="nama_shift" value="{{ old('nama_shift') }}" class="mt-1 w-full border rounded-lg px-3 py-2" required>
        ...
        <div>
            ...
            <input name="toleransi_terlambat" type="number" value="{{ old('toleransi_terlambat', 0) }}" class="mt-1 w-full border rounded-lg px-3 py-2">
        ...
        <div class="time-input">
            ...
            <input name="jam_masuk" type="time" value="{{ old('jam_masuk', '08:00') }}" class="mt-1 w-full border rounded-lg px-3 py-2" required>
            @error('jam_masuk')<div class="text-sm text-red-600 mt-1">{{ $message }}</div>@enderror
        </div>
        <div class="time-input">
            <label class="text-sm font-medium">Jam Keluar</label>
            <input name="jam_keluar" type="time" value="{{ old('jam_keluar', '17:00') }}" class="mt-1 w-full border rounded-lg px-3 py-2" required>
