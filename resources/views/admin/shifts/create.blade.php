...
<div class="max-w-3xl mx-auto">
    <h1 class="text-2xl font-semibold text-gray-900 tracking-tight mb-6">Tambah Shift</h1>

    <form method="POST" action="{{ route('admin.shifts.store') }}" class="bg-white border border-gray-100 rounded-2xl p-6 md:p-8 shadow-sm space-y-6">
        @csrf

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="md:col-span-2">
                <label class="flex items-center gap-3 cursor-pointer group">
                    <input type="checkbox" name="is_flexible" id="is_flexible" value="1" class="rounded border-gray-300 text-gray-900 shadow-sm focus:border-gray-900 focus:ring focus:ring-gray-900/10 focus:ring-opacity-50 transition" {{ old('is_flexible') ? 'checked' : '' }}>
                    <div>
            ...
            <div>
                ...
                <input name="nama_shift" value="{{ old('nama_shift') }}" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-gray-900/5 focus:border-gray-900 transition" required placeholder="Contoh: Shift Pagi">
            ...
            <div>
                ...
                <input name="toleransi_terlambat" type="number" value="{{ old('toleransi_terlambat', 0) }}" class="w-full border-gray-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-gray-900/5 focus:border-gray-900 transition">
            ...
            <div class="time-input">
                ...
                <input name="jam_masuk" type="time" value="{{ old('jam_masuk', '08:00') }}" class="w-full border-gray-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-gray-900/5 focus:border-gray-900 transition" required>
            ...
            <div class="time-input">
                ...
                <input name="jam_keluar" type="time" value="{{ old('jam_keluar', '17:00') }}" class="w-full border-gray-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-gray-900/5 focus:border-gray-900 transition" required>
                @error('jam_keluar')<div class="text-sm text-red-600 mt-1">{{ $message }}</div>@enderror
            </div>

            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 mb-2">Deskripsi</label>
                <textarea name="deskripsi" class="w-full border-gray-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-gray-900/5 focus:border-gray-900 transition" rows="3" placeholder="Deskripsi shift">{{ old('deskripsi') }}</textarea>
        ...
        <div class="flex items-center gap-3 pt-4 border-t border-gray-50 mt-6">
            <button class="px-6 py-2.5 rounded-xl bg-gray-900 text-white font-medium shadow-sm hover:bg-gray-800 transition">Simpan Shift</button>
