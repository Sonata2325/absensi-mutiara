...
<div class="max-w-4xl mx-auto">
    <h1 class="text-2xl font-semibold text-gray-900 tracking-tight mb-6">Tambah Karyawan</h1>

    <form method="POST" action="{{ route('admin.karyawan.store') }}" class="bg-white border border-gray-100 rounded-2xl p-6 md:p-8 shadow-sm space-y-6">
        @csrf

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Nama Lengkap</label>
                <input name="name" value="{{ old('name') }}" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-gray-900/5 focus:border-gray-900 transition" required placeholder="Masukkan nama lengkap">
                @error('name')<div class="text-sm text-red-600 mt-1">{{ $message }}</div>@enderror
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Nomor Telepon</label>
                <input name="phone" value="{{ old('phone') }}" class="w-full border-gray-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-gray-900/5 focus:border-gray-900 transition" required placeholder="08xxxxxxxxxx">
                @error('phone')<div class="text-sm text-red-600 mt-1">{{ $message }}</div>@enderror
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Password</label>
                <input name="password" type="password" class="w-full border-gray-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-gray-900/5 focus:border-gray-900 transition" required placeholder="••••••••">
            ...
            <div>
                ...
                <select name="position_id" class="w-full border-gray-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-gray-900/5 focus:border-gray-900 transition bg-white">
                    ...
                    @foreach($positions as $p)
                        <option value="{{ $p->id }}" @selected(old('position_id') == $p->id)>{{ $p->nama_posisi }}</option>
                    @endforeach
                </select>
        ...
        <div>
            ...
            <textarea name="alamat" class="w-full border-gray-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-gray-900/5 focus:border-gray-900 transition" rows="3" placeholder="Masukkan alamat lengkap">{{ old('alamat') }}</textarea>
        ...
        <div>
            ...
            <input name="kontak_darurat" value="{{ old('kontak_darurat') }}" class="w-full border-gray-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-gray-900/5 focus:border-gray-900 transition" placeholder="Nama - Nomor Telepon">
        ...
        <div class="flex items-center gap-3 pt-4 border-t border-gray-50 mt-6">
            <button class="px-6 py-2.5 rounded-xl bg-gray-900 text-white font-medium shadow-sm hover:bg-gray-800 transition">Simpan Karyawan</button>
