...
@section('content')
<h1 class="text-2xl font-bold mb-6">Tambah Posisi</h1>

<form method="POST" action="{{ route('admin.positions.store') }}" class="bg-white border rounded-2xl p-6 space-y-4">
    @csrf

    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            <label class="text-sm font-medium">Nama Posisi</label>
            <input name="nama_posisi" value="{{ old('nama_posisi') }}" class="mt-1 w-full border rounded-lg px-3 py-2" required>
        ...
        <div>
            ...
            <input name="kode_posisi" value="{{ old('kode_posisi') }}" class="mt-1 w-full border rounded-lg px-3 py-2" required>
