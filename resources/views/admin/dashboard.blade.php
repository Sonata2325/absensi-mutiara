...
<div class="flex flex-col gap-6">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            ...
            <p class="text-gray-500 mt-1">Overview statistik kehadiran hari ini</p>
        </div>
        <div class="flex items-center gap-3 bg-white px-4 py-2 rounded-xl border border-gray-100 shadow-sm">
            ...
            <span class="text-sm font-medium text-gray-900">{{ now()->locale('id')->isoFormat('dddd, D MMMM Y') }}</span>
        </div>
    </div>

    <!-- Quick Actions -->
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4">
        <a href="{{ route('admin.attendance.monitor') }}" class="bg-white p-4 md:p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:border-blue-100 transition group">
        ...
        <a href="{{ route('admin.karyawan.index') }}" class="bg-white p-4 md:p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:border-blue-100 transition group">
        ...
        <a href="{{ route('admin.leave.index') }}" class="bg-white p-4 md:p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:border-blue-100 transition group">
        ...
        <a href="{{ route('admin.reports.index') }}" class="bg-white p-4 md:p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:border-blue-100 transition group">
            <div class="w-10 h-10 md:w-12 md:h-12 mx-auto bg-orange-50 rounded-xl flex items-center justify-center text-orange-600 mb-3 group-hover:scale-110 transition">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 md:w-6 md:h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-lg font-bold text-gray-900">Grafik Kehadiran 7 Hari Terakhir</h2>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                <thead class="bg-gray-50 text-gray-500 uppercase tracking-wider text-xs">
                    <tr>
                        <th class="p-4 rounded-l-xl">Tanggal</th>
                        <th class="p-4">Hadir</th>
                        <th class="p-4 rounded-r-xl">Terlambat</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    @forelse($chart as $row)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="p-4 font-medium text-gray-900 whitespace-nowrap">{{ \Carbon\Carbon::parse($row['tanggal'])->locale('id')->isoFormat('dddd, D MMM') }}</td>
                            <td class="p-4 min-w-[150px]">
                                <div class="flex items-center gap-2">
                                    <div class="w-full max-w-[100px] bg-gray-100 rounded-full h-2">
                                        <div class="bg-green-500 h-2 rounded-full" style="width: {{ $totalEmployees > 0 ? ($row['hadir'] / $totalEmployees) * 100 : 0 }}%"></div>
                                    </div>
                                    <span class="text-gray-600">{{ $row['hadir'] }}</span>
                                </div>
                            </td>
                            <td class="p-4 min-w-[150px]">
                                <div class="flex items-center gap-2">
                                    <div class="w-full max-w-[100px] bg-gray-100 rounded-full h-2">
                                        <div class="bg-yellow-500 h-2 rounded-full" style="width: {{ $totalEmployees > 0 ? ($row['terlambat'] / $totalEmployees) * 100 : 0 }}%"></div>
                                    </div>
                                    <span class="text-gray-600">{{ $row['terlambat'] }}</span>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="3" class="p-4 text-center text-gray-500">Belum ada data minggu ini.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
